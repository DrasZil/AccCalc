...
export default function NoteInterestPage() {
    ...
    const result = useMemo(() => {
        ...
        return {
            error: "Principal and time must be greater than 0, and rate cannot be negative.",
        };
        }
        ...
        timeExplanation = `${timeNumber} month(s) = ${convertedTime} year(s)`;
        } else if (timeUnit === "days") {
        convertedTime = timeNumber / 360;
        timeExplanation = `${timeNumber} day(s) = ${convertedTime} year(s) using a 360-day year`;
        }
        ...
        steps: [
            ...
            <>Rate = {rateNumber.toFixed(2)}% = {rateDecimal.toFixed(4)}</>,
            <>{timeExplanation}</>,
            <>
            Interest = {principalNumber} × {rateDecimal.toFixed(4)} × {convertedTime.toFixed(4)} = {interest.toFixed(2)}
            </>,
            <>
            Maturity Value = {principalNumber} + {interest.toFixed(2)} = {maturityValue.toFixed(2)}
            </>,
        ],
        };
    ...
    return (
        <CalculatorPageLayout 
            ...
            using principal, annual rate, and time"
            inputSection={
                <div className = "space-y-4">
                    ...
                    <SectionCard>
                        ...
                        <select 
                            ...
                            px-4 py-3 text-white outline-none transition focus:border-green-400/40

...
export default function PartnershipSalaryInterestPage() {
    ...
    const result = useMemo(() => {
        ...
        const interestShareA =
            parsedPartnerAAverageCapital * (parsedInterestRate / 100);
        const interestShareB =
            parsedPartnerBAverageCapital * (parsedInterestRate / 100);
        ...
        const ratioTotal =
            parsedPartnerARemainderRatio + parsedPartnerBRemainderRatio;
        ...
        return {
            ...
            steps: [
                ...
                `Remainder = ${formatPHP(parsedPartnershipAmount)} - total salary and interest allowances of ${formatPHP(totalAppropriation)} = ${formatPHP(remainder)}`,
    ...
    return (
        <CalculatorPageLayout
            ...
            description="Allocate partnership income after salary allowances, interest on capital, and the remainder ratio. This is a common partnership-accounting topic in university accounting courses."
            inputSection={
                <SectionCard>
                    <InputGrid columns={2}>
                        <InputCard label="Partnership Net Income or Loss" value={partnershipAmount} onChange={setPartnershipAmount} placeholder="150000" />
                        <InputCard label="Partner A Salary Allowance" value={partnerASalary} onChange={setPartnerASalary} placeholder="30000" />
                        <InputCard label="Partner B Salary Allowance" value={partnerBSalary} onChange={setPartnerBSalary} placeholder="25000" />
                        <InputCard label="Partner A Average Capital" value={partnerAAverageCapital} onChange={setPartnerAAverageCapital} placeholder="200000" />
                        <InputCard label="Partner B Average Capital" value={partnerBAverageCapital} onChange={setPartnerBAverageCapital} placeholder="180000" />
                        <InputCard label="Interest Rate (%)" value={interestRate} onChange={setInterestRate} placeholder="10" />
                        <InputCard label="Partner A Remainder Ratio" value={partnerARemainderRatio} onChange={setPartnerARemainderRatio} placeholder="3" />
                        <InputCard label="Partner B Remainder Ratio" value={partnerBRemainderRatio} onChange={setPartnerBRemainderRatio} placeholder="2" />
                    </InputGrid>
                </SectionCard>
            }
            resultSection={
                result && "error" in result ? (
                    ...
                    <ResultGrid columns={2}>
                        ...
                        <ResultCard title="Partner A Interest Allowance" value={formatPHP(result.interestShareA)} />
                        <ResultCard title="Partner B Interest Allowance" value={formatPHP(result.interestShareB)} />

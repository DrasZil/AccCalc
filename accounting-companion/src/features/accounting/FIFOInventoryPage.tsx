...
export default function FIFOInventoryPage(){
    ...
    const result = useMemo(() => {
        ...
        const numericValues = [bUnits, bCost, p1Units, p1Cost, 
            p2Units, p2Cost, sold
        ];
        ...
        for (const layer of remainingLayers) {
            ...
            if (unitsFromLayer > 0){
                issueSteps.push(
                    `${unitsFromLayer} unit(s) × ${formatPHP(layer.unitCost)} from ${layer.label} = ${formatPHP(layerCost)}`
                );
            }
        ...
        const remainingInventoryLines = remainingLayers
            ...
            .map(
                (layer) =>
                `${layer.label}: ${layer.units} unit(s) × ${formatPHP(layer.unitCost)} = ${formatPHP(
        ...
        return {
            ...
            steps: [
                ...
                `Units sold = ${sold}`,
                "Cost of Goods Sold is taken from oldest inventory layers first:",
                ...issueSteps,

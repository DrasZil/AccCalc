...
export default function WeightedAverageInventoryPage() {
    ...
    const result = useMemo(() => {
        ...
        const totalCostAvailable = bUnits * bCost + p1Units * p1Cost + p2Units * p2Cost;
        ...
        return {
            ...
            steps: [
                `Beginning Inventory Cost = ${bUnits} unit(s) x ${formatPHP(bCost)} = ${formatPHP(
                    bUnits * bCost
                )}`,
                `Purchase 1 Cost = ${p1Units} unit(s) x ${formatPHP(p1Cost)} = ${formatPHP(
                    p1Units * p1Cost
                )}`,
                `Purchase 2 Cost = ${p2Units} unit(s) x ${formatPHP(p2Cost)} = ${formatPHP(
                    p2Units * p2Cost
                )}`,
                ...
                `Cost of Goods Sold = ${sold} x ${formatPHP(weightedAverageUnitCost)} = ${formatPHP(
                    cogs
                )}`,
                `Ending Units = ${totalUnitsAvailable} - ${sold} = ${endingUnits}`,
                `Ending Inventory = ${endingUnits} x ${formatPHP(weightedAverageUnitCost)} = ${formatPHP(
                    endingInventory
                )}`,
            ],
            interpretation: `The weighted average method spreads total available cost evenly across all ${totalUnitsAvailable} units. That gives an average unit cost of ${formatPHP(weightedAverageUnitCost)}, so ${sold} units sold carry ${formatPHP(cogs)} to cost of goods sold and the remaining ${endingUnits} units stay in ending inventory at ${formatPHP(endingInventory)}.`,
        };
    }, [
        beginningUnits,
        beginningCost,
            ...
            resultSection={
                result && "error" in result ? (
                    ...
                    <ResultGrid columns={2}>
                        ...
                        <ResultCard title="Ending Units" value={String(result.endingUnits)} />
                        <ResultCard

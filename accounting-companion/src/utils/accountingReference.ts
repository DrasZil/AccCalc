...
export type AccountReference = {
    name: string;
    type: "Asset" | "Liability" | "Equity" | "Revenue" | "Contra Revenue" | "Expense" | "Contra Asset";
...
export const ACCOUNT_REFERENCE_DATA: AccountReference[] = [
...
export function searchAccountReferences(query: string) {
    const normalizedQuery = normalizeText(query);
    if (!normalizedQuery) return ACCOUNT_REFERENCE_DATA.slice(0, 8);

...
export const FLEXIBLE_NUMBER_CAPTURE_PATTERN =
    String.raw`(?:\((?:[\u20B1$]\s*)?\d[\d,]*(?:\.\d+)?\)|-?(?:[\u20B1$]\s*)?\d[\d,]*(?:\.\d+)?)(?:\s*%)?`;

export function parseLooseNumber(
    value: string | number | null | undefined,
    options?: { percentAsFraction?: boolean }
): number | null {
...
export function detectStructuredValueKind(value: string): StructuredValueKind {
    const trimmed = value.trim();
    if (!trimmed) return "text";
    if (/%/.test(trimmed)) return "percent";
    if (/[\u20B1$]|(?:\bphp\b)|(?:\bpeso(?:s)?\b)/i.test(trimmed)) return "money";
    ...
    if (parseLooseNumber(trimmed) !== null) return "number";
    return "text";
}

export function normalizeStructuredFieldValue(value: string): string {
    ...
    if (kind === "quantity") {
        const unitMatch = value.trim().match(
            /\b(?:units?|hours?|days?|years?|months?|kg|g|cm|mm|m|km)\b/i
        );

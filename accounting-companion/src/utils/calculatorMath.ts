...
type SpecialOrderDecisionParams = {
    specialOrderUnits: number;
    specialOrderPricePerUnit: number;
    variableCostPerUnit: number;
    incrementalFixedCosts?: number;
};

type MakeOrBuyDecisionParams = {
    unitsNeeded: number;
    variableManufacturingCostPerUnit: number;
    avoidableFixedCosts?: number;
    purchasePricePerUnit: number;
};

type SellProcessFurtherParams = {
    units: number;
    salesValueAtSplitoffPerUnit: number;
    salesValueAfterProcessingPerUnit: number;
    separableProcessingCostPerUnit: number;
};

type ConstrainedResourceProductMixParams = {
    sellingPricePerUnit: number;
    variableCostPerUnit: number;
    constrainedResourceUnitsPerProduct: number;
    constrainedResourceAvailableUnits: number;
};

type BudgetVarianceAnalysisParams = {
    actualResultAmount: number;
    flexibleBudgetAmount: number;
    staticBudgetAmount: number;
};

type MovingAverageForecastParams = {
    period1Demand: number;
    period2Demand: number;
    period3Demand: number;
    weight1Percent: number;
    weight2Percent: number;
    weight3Percent: number;
};
...
export function computeCapitalRationingSelection(projects: Array<{
    ...
    return {
        ...
        selectionMethod: exactSearchEvaluated ? "exact-combination" as const : "greedy-pi-fallback" as const,
        selectedProjects,
        totalInvestment,
        totalNpv,
        remainingBudget,
...
export function computeSpecialOrderDecision({
    specialOrderUnits,
    specialOrderPricePerUnit,
    variableCostPerUnit,
    incrementalFixedCosts = 0,
}: SpecialOrderDecisionParams) {
    const incrementalRevenue = safeMultiply(
        specialOrderUnits,
        specialOrderPricePerUnit
    );
    const incrementalVariableCosts = safeMultiply(
        specialOrderUnits,
        variableCostPerUnit
    );
    const incrementalCosts = safeAdd(
        incrementalVariableCosts,
        incrementalFixedCosts
    );
    const incrementalProfit = safeSubtract(
        incrementalRevenue,
        incrementalCosts
    );
    const minimumAcceptablePricePerUnit =
        specialOrderUnits <= 0
            ? Number.NaN
...
export function computeMakeOrBuyDecision({
    unitsNeeded,
    variableManufacturingCostPerUnit,
    avoidableFixedCosts = 0,
    purchasePricePerUnit,
}: MakeOrBuyDecisionParams) {
    const relevantMakeCost = safeAdd(
        safeMultiply(unitsNeeded, variableManufacturingCostPerUnit),
        avoidableFixedCosts
    );
    ...
    const maximumAcceptablePurchasePricePerUnit =
        unitsNeeded <= 0
            ? Number.NaN
...
export function computeSellProcessFurther({
    units,
    salesValueAtSplitoffPerUnit,
    salesValueAfterProcessingPerUnit,
    separableProcessingCostPerUnit,
}: SellProcessFurtherParams) {
    const incrementalRevenueFromProcessing = safeMultiply(
        units,
        safeSubtract(
            salesValueAfterProcessingPerUnit,
            salesValueAtSplitoffPerUnit
    ...
    const separableProcessingCosts = safeMultiply(
        units,
        separableProcessingCostPerUnit
    );
    const incrementalProfitFromProcessing = safeSubtract(
        incrementalRevenueFromProcessing,
        separableProcessingCosts
    );
    const minimumFurtherProcessingPricePerUnit = safeAdd(
        salesValueAtSplitoffPerUnit,
        separableProcessingCostPerUnit
    );
...
export function computeConstrainedResourceProductMix({
    sellingPricePerUnit,
    variableCostPerUnit,
    constrainedResourceUnitsPerProduct,
    constrainedResourceAvailableUnits,
}: ConstrainedResourceProductMixParams) {
    const contributionMarginPerUnit = safeSubtract(
        sellingPricePerUnit,
        variableCostPerUnit
    );
    const contributionMarginPerConstraintUnit =
        constrainedResourceUnitsPerProduct <= 0
            ? Number.NaN
            : safeDivide(
                  contributionMarginPerUnit,
                  constrainedResourceUnitsPerProduct
              );
    const maximumUnitsFromConstraint =
        constrainedResourceUnitsPerProduct <= 0
            ? Number.NaN
            : safeDivide(
                  constrainedResourceAvailableUnits,
    ...
    const totalContributionMarginAtConstraint =
        Number.isFinite(maximumUnitsFromConstraint)
            ? safeMultiply(
                  contributionMarginPerUnit,
...
export function computeBudgetVarianceAnalysis({
    actualResultAmount,
    flexibleBudgetAmount,
    staticBudgetAmount,
}: BudgetVarianceAnalysisParams) {
    const spendingVariance = safeSubtract(
        actualResultAmount,
        flexibleBudgetAmount
    );
    const activityVariance = safeSubtract(
        flexibleBudgetAmount,
        staticBudgetAmount
    );
    const totalBudgetVariance = safeSubtract(
        actualResultAmount,
        staticBudgetAmount
    );
    ...
    return {
        ...
        activityVariance,
        totalBudgetVariance,
        spendingVarianceLabel:
            spendingVariance > 0
                ? "Unfavorable"
                : spendingVariance < 0
                  ? "Favorable"
                  : "On target",
        activityVarianceLabel:
            activityVariance > 0
                ? "Above static plan"
                : activityVariance < 0
                  ? "Below static plan"
...
export function computeMovingAverageForecast({
    ...
    weight2Percent,
    weight3Percent,
}: MovingAverageForecastParams) {
    const simpleMovingAverageForecast = safeDivide(
        safeAdd(safeAdd(period1Demand, period2Demand), period3Demand),
        3
    );
    const totalWeightPercent = safeAdd(
        safeAdd(weight1Percent, weight2Percent),
        weight3Percent
    );
    const weightedMovingAverageForecast =
        totalWeightPercent === 0
            ...
            : safeDivide(
                  safeAdd(
                      safeAdd(
                          safeMultiply(period1Demand, weight1Percent),
                          safeMultiply(period2Demand, weight2Percent)
                      ),
                      safeMultiply(period3Demand, weight3Percent)
                  ),
                  totalWeightPercent
              );
...
export function computeAuditMisstatementEvaluation({
    tolerableMisstatement,
    ...
    qualitativeConcernCount: number;
}) {
    const upperMisstatementLimit = safeAdd(
        projectedMisstatement,
        allowanceForSamplingRisk
    );
    ...
    const adjustedRiskIndex = roundTo(
        safeAdd(utilizationRate, qualitativePenalty),
        2
    );
    ...
    return {
        ...
        aboveClearlyTrivial,
        conclusion:
            upperMisstatementLimit > tolerableMisstatement || qualitativeConcernCount >= 3
...
export function computeSegregationOfDutiesConflict({
    ...
    authorizationCustodyConflict: number;
    custodyRecordingConflict: number;
    recordingReconciliationConflict: number;
    privilegedAccessConflict: number;
    compensatingReviewStrength: number;
}) {
    const rawConflictScore = safeAdd(
        safeAdd(authorizationCustodyConflict, custodyRecordingConflict),
        safeAdd(recordingReconciliationConflict, privilegedAccessConflict)
    );
    const mitigationCredit = roundTo(
        safeMultiply(Math.max(safeSubtract(compensatingReviewStrength, 1), 0), 0.9),
        2
    );
    const netConflictScore = roundTo(
        Math.max(safeSubtract(rawConflictScore, mitigationCredit), 1),
        2
    );
    const conflictRows = [
        {
...
export function computeGovernanceEscalationPlan({
    issueSeverity,
    overrideRisk,
    stakeholderExposure,
    ...
    const escalationScore = safeAdd(
        safeAdd(issueSeverity, overrideRisk),
        safeAdd(stakeholderExposure, safeAdd(evidenceGap, oversightGap))
    );
    ...
    return {
        ...
        preserveEvidence,
        urgency:
            escalationScore >= 12 || overrideRisk === 3
                ? "Immediate"
                : escalationScore >= 9
                  ? "Prompt"
...
export function computeRevenueAllocationAnalysis({
    transactionPrice,
    ...
    standaloneSellingPriceA: number;
    standaloneSellingPriceB: number;
    percentSatisfiedA: number;
    percentSatisfiedB: number;
    considerationReceived: number;
}) {
    const totalStandaloneSellingPrice = safeAdd(
        standaloneSellingPriceA,
        standaloneSellingPriceB
    );
    const allocationA = safeMultiply(
        transactionPrice,
        safeDivide(standaloneSellingPriceA, totalStandaloneSellingPrice)
    );
    const allocationB = safeMultiply(
        transactionPrice,
        safeDivide(standaloneSellingPriceB, totalStandaloneSellingPrice)
    );
    ...
    const contractLiability = Math.max(
        safeSubtract(considerationReceived, revenueRecognized),
        0
    );
    const contractAsset = Math.max(
        safeSubtract(revenueRecognized, considerationReceived),
        0
    );
...
export function computeTaxableIncomeBridge({
    accountingIncome,
    permanentAdditions,
    permanentDeductions,
    ...
    permanentAdditions: number;
    permanentDeductions: number;
    taxableTemporaryDifferences: number;
    deductibleTemporaryDifferences: number;
    nolDeduction: number;
    taxRatePercent: number;
}) {
    const taxableIncomeBeforeNol = safeSubtract(
        safeAdd(accountingIncome, safeAdd(permanentAdditions, taxableTemporaryDifferences)),
        safeAdd(permanentDeductions, deductibleTemporaryDifferences)
    );
    const taxableIncome = Math.max(safeSubtract(taxableIncomeBeforeNol, nolDeduction), 0);
    const currentTaxExpense = safeMultiply(taxableIncome, percentToDecimal(taxRatePercent));
    const deferredTaxLiability = safeMultiply(
        Math.max(taxableTemporaryDifferences, 0),
        percentToDecimal(taxRatePercent)
    );
    const deferredTaxAsset = safeMultiply(
        Math.max(deductibleTemporaryDifferences, 0),
        percentToDecimal(taxRatePercent)
    );
...
export function computeTargetCostingGap({
    targetSellingPrice,
    ...
    expectedUnitVolume: number;
    currentEstimatedCostPerUnit: number;
    committedCostReductionPerUnit: number;
}) {
    const targetProfitPerUnit = safeMultiply(
        targetSellingPrice,
        percentToDecimal(targetProfitMarginPercent)
    );
    const allowableCostPerUnit = safeSubtract(targetSellingPrice, targetProfitPerUnit);
    const revisedCostPerUnit = safeSubtract(
        currentEstimatedCostPerUnit,
        committedCostReductionPerUnit
    );
    ...
    const requiredReductionPercent = safeMultiply(
        safeDivide(Math.max(remainingCostGapPerUnit, 0), revisedCostPerUnit),
        100
    );
...
export function computeAuditEvidenceProgram({
    assertionRisk,
    evidenceReliability,
    evidenceRelevance,
    coverageDepth,
    ...
    assertionRisk: number;
    evidenceReliability: number;
    evidenceRelevance: number;
    coverageDepth: number;
    contradictionCount: number;
}) {
    const evidenceStrengthAverage = safeDivide(
        safeAdd(safeAdd(evidenceReliability, evidenceRelevance), coverageDepth),
        3
    );
    const contradictionPenalty = safeMultiply(contradictionCount, 0.6);
    const residualEvidenceGap = roundTo(
        Math.max(safeSubtract(assertionRisk, safeSubtract(evidenceStrengthAverage, contradictionPenalty)), 0),
        2
    );
...
export function computeAuditMaterialityPlan({
    benchmarkAmount,
    ...
    benchmarkAmount: number;
    planningMaterialityPercent: number;
    performanceMaterialityPercent: number;
    clearlyTrivialPercent: number;
    expectedMisstatement: number;
    identifiedUncorrectedMisstatement: number;
}) {
    const planningMateriality = safeMultiply(
        benchmarkAmount,
        percentToDecimal(planningMaterialityPercent)
    );
    const performanceMateriality = safeMultiply(
        planningMateriality,
        percentToDecimal(performanceMaterialityPercent)
    );
    const clearlyTrivialThreshold = safeMultiply(
        planningMateriality,
        percentToDecimal(clearlyTrivialPercent)
    );
    const aggregateMisstatementPressure = safeAdd(
        expectedMisstatement,
        identifiedUncorrectedMisstatement
    );
    const remainingPerformanceCushion = safeSubtract(
        performanceMateriality,
        aggregateMisstatementPressure
    );
    const pressureRatio = safeMultiply(
        safeDivide(aggregateMisstatementPressure, performanceMateriality),
        100
    );
...
export function computeIncomeTaxPayableReview({
    taxableIncome,
    ...
    const grossIncomeTaxDue = safeMultiply(
        Math.max(taxableIncome, 0),
        percentToDecimal(incomeTaxRatePercent)
    );
    const totalCreditsAndPayments = safeAdd(
        safeAdd(withholdingCredits, quarterlyPayments),
        refundableCredits
    );

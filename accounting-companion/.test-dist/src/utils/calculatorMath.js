...
export function computeCapitalRationingSelection(projects, capitalBudget) {
    ...
    const selectedProjects = rankedProjects.filter((project) => {
        if (project.initialInvestment > remainingBudget)
            return false;
...
export function computeConfidenceInterval({ sampleMean, sampleStandardDeviation, sampleSize, confidenceLevelPercent, }) {
    const zCritical = confidenceLevelPercent >= 99
        ? 2.576

...
export function getResultValueTone(value) {
    ...
    const looksMostlyNumeric = numericCount > 0 &&
        alphaCount <= Math.max(4, Math.floor(trimmed.length * 0.18)) &&
        words.length <= 4;
    if (looksMostlyNumeric)
        return "numeric";
    if (hasSentenceLength)
        return "sentence";
    return "label";
}
...
export function getResultValueSegments(value) {

import { detectStructuredValueKind, normalizeStructuredFieldValue, } from "../../../../utils/numberParsing.js";
const CLEANUPS = [
...
export function normalizeAccountingWorksheetText(text) {
    return text
        .replace(/\r/g, "")
        .split("\n")
        .map((line) => CLEANUPS.reduce((current, [pattern, replacement]) => current.replace(pattern, replacement), line.replace(/[|]/g, " ").replace(/\s{2,}/g, " ").trim()))
        .filter(Boolean)
        .join("\n");
}
export function extractAccountingFields(text) {
    ...
    FIELD_PATTERNS.forEach((field) => {
        ...
        fields.push({
            ...
            valueKind,
            sourceLine,
            needsReview: Boolean(sourceLine && rawValue !== sourceLine) ||

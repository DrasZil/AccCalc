...
export function recommendScanRoutes(text, kind, pageType = "unknown") {
    ...
    const ranked = ROUTE_RULES.map((rule) => {
        ...
        const score = baseMatches * 11 +
            bonusMatches * 4 +
            (rule.kindBoosts?.[kind] ?? 0) +
            (rule.pageTypeBoosts?.[pageType] ?? 0);
        const confidence = score >= 28 ? "high" : score >= 16 ? "moderate" : score >= 8 ? "low" : null;
        return confidence
            ? {
                path: rule.path,
                label: rule.label,
                category: rule.category,
                reason: baseMatches > 0
        ...
        return [
            {
                ...
                reason: "The scan is still too ambiguous for a specialized calculator, so Smart Solver is the safest next step.",
                score: 12,
                confidence: "low",
            },
        ];
    ...
    if (ranked[0]?.score < 22) {
        ranked.push({
            ...
            label: "Smart Solver",
            category: "Smart Tools",
            reason: "Use Smart Solver if the scan is incomplete, mixed-topic, or still needs a human-style interpretation before opening a calculator.",
            score: 18,
            confidence: "moderate",
        });
    }
    if (ranked.length > 1 && ranked[0].score - ranked[1].score <= 4) {
        ranked.push({
            ...
            label: "Smart Solver",
            category: "Smart Tools",
            reason: "The two strongest route matches are still close together, so Smart Solver is a safer fallback if the scan mixes topics or some labels may be wrong.",

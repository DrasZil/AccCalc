...
import { analyzeSmartInput, FIELD_META, INITIAL_FIELDS, } from "../../../smart/smartSolver.engine.js";
import { detectStructuredValueKind, FLEXIBLE_NUMBER_CAPTURE_PATTERN, normalizeStructuredFieldValue, } from "../../../../utils/numberParsing.js";
import { extractAccountingFields, normalizeAccountingWorksheetText, } from "../accounting/accountingFieldExtractor.js";
...
function extractUnits(text) {
    const matches = text.match(/\b(?:kg|g|cm|mm|m|km|%|days?|years?|months?|hours?|php|peso(?:s)?|units?)\b|₱/gi) ?? [];
...
function extractValues(text) {
    const lines = text
        ...
        .filter(Boolean);
    const matches = [];
    const labeledLinePattern = new RegExp(String.raw `^([A-Za-z][A-Za-z0-9 /(),-]{2,42}?)(?:\s*[:=-]\s*|\s{2,})(${FLEXIBLE_NUMBER_CAPTURE_PATTERN}(?:\s*(?:units?|kg|g|cm|mm|m|km|days?|years?|months?|hours?))?)$`, "i");
    const numberPattern = new RegExp(`${FLEXIBLE_NUMBER_CAPTURE_PATTERN}(?:\\s*(?:units?|kg|g|cm|mm|m|km|days?|years?|months?|hours?))?`, "gi");
...
function extractStructuredCandidates(text) {
    const lines = text
        ...
        .map((line) => line.trim())
        .filter(Boolean);
    const pattern = new RegExp(String.raw `^([A-Za-z][A-Za-z0-9 /(),-]{2,42}?)(?:\s*[:=-]\s*|\s{2,})(${FLEXIBLE_NUMBER_CAPTURE_PATTERN}(?:\s*(?:units?|kg|g|cm|mm|m|km|days?|years?|months?|hours?))?)$`, "i");
    const seen = new Set();
    const fields = [];
    for (const line of lines) {
...
function detectLikelyIssues(text, flaggedValues, structuredFields) {
    const issues = [];
    const suspiciousCompactTokens = text.match(/\b\d+[A-Za-z]{1,3}\d*\b|\b[A-Za-z]{1,2}\d+[A-Za-z]*\b/g) ?? [];
    ...
    if (suspiciousCompactTokens.length > 0) {
        issues.push(`Review tightly merged OCR tokens such as ${suspiciousCompactTokens
            .slice(0, 3)
            .join(", ")} because numbers and labels may still be stuck together.`);
    }
    if (structuredFields.some((field) => field.needsReview)) {
        issues.push("Some structured values still need review because OCR normalization changed their raw shape or the label-value pairing is only moderately confident.");
    }
...
function buildSmartStructuredFields(text) {
    const smartAnalysis = analyzeSmartInput({ ...INITIAL_FIELDS }, text);
    const structuredFields = smartAnalysis.extractedEntries.map(([key, value]) => ({
        key,
        ...
        confidence: !smartAnalysis.best
            ? 62
            : Math.max(62, Math.min(92, smartAnalysis.best?.score ?? 62)),
        needsReview: smartAnalysis.warnings.length > 0 ||
...
export function parseOcrText(text, ocrConfidence) {
    ...
    const accountingPageType = accountingClassification.pageType;
    const isAccountingWorksheet = accountingFields.length >= 3 || accountingPageType !== "unknown";
    ...
    const structuredFields = isAccountingWorksheet
        ...
        : smartStructured.structuredFields.length > 0
            ...
            : genericStructuredFields.length > 0
                ...
                : undefined;
    const likelyIssues = detectLikelyIssues(cleanedText, cleanup.flaggedValues, structuredFields ?? []);
    const fallbackRecommendations = recommendScanRoutes(isAccountingWorksheet ? normalizedAccountingText : cleanedText, kind, accountingPageType);
    const solverRecommendations = smartStructured.smartAnalysis.best
        ? [
            {
                ...
                confidence: getRecommendationConfidence(smartStructured.smartAnalysis.best.score),
            },
        ...
        : [];
    const shouldPreferSolverRecommendations = solverRecommendations.length > 0 &&
    ...
    const studyRecommendations = recommendStudyTopicsFromText(isAccountingWorksheet ? normalizedAccountingText : cleanedText, primaryRecommendation?.path);
    const parseConfidence = Math.max(25, Math.min(100, ocrConfidence -
        (kind === "unknown" ? 22 : 0) +
    ...
    const suggestedIntent = kind === "accounting-worksheet"
        ? "Check my solution"
        : kind === "equation"
            ? "Open suggested tool"
    ...
    const routeGap = primaryRecommendation && secondRecommendation
        ? primaryRecommendation.score - secondRecommendation.score
        : null;
    const routeLooksAmbiguous = routeGap !== null &&
        (routeGap <= 4 ||
    ...
    if (routeLooksAmbiguous) {
        likelyIssues.push(`Route selection is still close between ${primaryRecommendation?.label} and ${secondRecommendation?.label}, so confirm the topic before trusting the first recommendation.`);
    }
    ...
    return {
        ...
        accounting: isAccountingWorksheet
            ? {
                ...
                extractionConfidence: Math.min(95, Math.round((accountingClassification.confidence + parseConfidence) / 2)),
                routeHint,

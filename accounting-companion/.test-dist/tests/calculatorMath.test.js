...
runTest("search indexes aliases, abbreviations, and typo-tolerant queries", () => {
    ...
    assert.equal(equityResults.some((result) => result.path === "/far/statement-of-changes-in-equity-builder"), true);
});
runTest("smart solver target intent prefers explicit reverse-solve wording", () => {
    ...
    assert.equal(suggestSolveTarget("inventory-budget", "compute required purchases"), "purchasesRequiredCost");
    assert.equal(suggestSolveTarget("sales-budget", "find the budgeted sales revenue"), "budgetedSalesAmount");
    assert.equal(suggestSolveTarget("operating-expense-budget", "find the cash operating expenses"), "cashOperatingExpenses");
    ...
    assert.equal(suggestSolveTarget("special-order-analysis", "find the minimum acceptable price"), "minimumAcceptablePricePerUnit");
    assert.equal(suggestSolveTarget("make-or-buy-analysis", "which option has the cost advantage"), "costAdvantageAmount");
    assert.equal(suggestSolveTarget("sell-or-process-further", "find incremental profit from processing"), "incrementalProfitFromProcessing");
    assert.equal(suggestSolveTarget("constrained-resource-product-mix", "compute contribution margin per constrained resource unit"), "contributionMarginPerConstraintUnit");
    assert.equal(suggestSolveTarget("budget-variance-analysis", "solve for spending variance"), "spendingVariance");
    assert.equal(suggestSolveTarget("moving-average-forecast", "weighted moving average forecast"), "weightedMovingAverageForecast");
});
...
runTest("new 7.0.0 solve definitions stay aligned with shared decision math", () => {
    const specialOrderSolve = specialOrderDecisionSolveDefinition.solve("minimumAcceptablePricePerUnit", {
        specialOrderUnits: 1000,
        specialOrderPricePerUnit: 90,
        variableCostPerUnit: 65,
        incrementalFixedCosts: 12000,
    ...
    const processFurtherSolve = sellProcessFurtherSolveDefinition.solve("incrementalProfitFromProcessing", {
        units: 5000,
        salesValueAtSplitoffPerUnit: 20,
        salesValueAfterProcessingPerUnit: 29,
        separableProcessingCostPerUnit: 6,
    });
    const constrainedMixSolve = constrainedResourceProductMixSolveDefinition.solve("contributionMarginPerConstraintUnit", {
        sellingPricePerUnit: 85,
        variableCostPerUnit: 55,
        constrainedResourceUnitsPerProduct: 4,
        constrainedResourceAvailableUnits: 2400,
    });
    const budgetVarianceSolve = budgetVarianceAnalysisSolveDefinition.solve("spendingVariance", {
        actualResultAmount: 528000,
        flexibleBudgetAmount: 510000,
        staticBudgetAmount: 495000,
    });
    const forecastSolve = movingAverageForecastSolveDefinition.solve("weightedMovingAverageForecast", {
        period1Demand: 920,
        period2Demand: 980,
        period3Demand: 1040,
        weight1Percent: 20,
...
runTest("search routes surface completion-pass FAR, operations, audit, tax, AIS, governance, RFBT, and strategic additions", () => {
    assert.equal(searchAppRoutes("qualifying asset avoidable interest")[0]?.path, "/far/borrowing-costs-capitalization");
    assert.equal(searchAppRoutes("cost plus pricing target margin")[0]?.path, "/operations/cost-plus-pricing");
    assert.equal(searchAppRoutes("audit assertion evidence working paper")[0]?.path, "/audit/assertion-evidence-planner");
    assert.equal(searchAppRoutes("excise tax local taxation incentives")[0]?.path, "/tax/excise-local-and-incentive-review");
    assert.equal(searchAppRoutes("erp crm business intelligence controls")[0]?.path, "/ais/enterprise-systems-control-mapper");
    assert.equal(searchAppRoutes("management override stakeholder ethics")[0]?.path, "/governance/ethics-decision-workspace");
    assert.equal(searchAppRoutes("insider trading corporate governance")[0]?.path, "/rfbt/securities-and-governance-review");
    assert.equal(searchAppRoutes("balanced scorecard learning and growth")[0]?.path, "/strategic/balanced-scorecard-workspace");
});
...
runTest("v12.4 academic expansion routes are discoverable through the catalog", () => {
    assert.equal(searchAppRoutes("audit projected misstatement allowance sampling risk")[0]?.path, "/audit/misstatement-evaluation-workspace");
    assert.equal(searchAppRoutes("segregation of duties custody recording privileged access")[0]?.path, "/ais/segregation-of-duties-conflict-matrix");
    assert.equal(searchAppRoutes("governance escalation management override audit committee")[0]?.path, "/governance/governance-escalation-planner");
});
runTest("v13 academic expansion routes are discoverable and routed", () => {
    assert.equal(searchAppRoutes("conceptual framework recognition measurement basis")[0]?.path, "/far/conceptual-framework-recognition");
    assert.equal(searchAppRoutes("standalone selling price performance obligation contract liability")[0]?.path, "/far/revenue-allocation-workspace");
    assert.equal(searchAppRoutes("taxable income permanent temporary difference deferred tax")[0]?.path, "/tax/taxable-income-bridge");
    assert.equal(searchAppRoutes("target costing")[0]?.path, "/strategic/target-costing-workspace");
    const smart = analyzeSmartInput({ ...INITIAL_FIELDS }, "The case asks for revenue recognition using standalone selling prices and performance obligations, then whether a contract liability remains.");
...
runTest("smart solver target suggestions understand transfer-pricing prompts", () => {
    ...
    assert.equal(suggestSolveTarget("borrowing-costs-capitalization", "how much avoidable interest should be capitalized"), "capitalizableBorrowingCost");
    assert.equal(suggestSolveTarget("cost-plus-pricing", "find the markup percent on cost"), "markUpPercent");
    assert.equal(suggestSolveTarget("audit-sampling-planner", "find the sample size"), "sampleSize");
    assert.equal(suggestSolveTarget("segmented-income-statement", "compute segment margin"), "segmentMargin");
    assert.equal(suggestSolveTarget("pert-project-estimate", "compute expected project time"), "expectedTime");
});
runTest("borrowing costs capitalization computes avoidable interest consistently", () => {
    ...
    assert.equal(Number(result.capitalizableBorrowingCost.toFixed(6)), Number((538333.3333333334).toFixed(6)));
});
...
runTest("smart solver routes bank reconciliation ahead of generic finance matches", () => {
    const analysis = analyzeSmartInput({ ...INITIAL_FIELDS }, "Prepare a bank reconciliation. Bank balance is 120000, book balance is 118500, deposits in transit are 8000, outstanding checks are 6500, service charges are 500, NSF checks are 1200, and interest income is 300.");
...
runTest("smart solver keeps cash-budget support routes secondary instead of contaminating the primary route", () => {
    const analysis = analyzeSmartInput({ ...INITIAL_FIELDS }, "Prepare a cash budget for April with beginning cash 50000, cash collections of 180000, cash disbursements of 210000, and a minimum cash balance of 25000. We may need financing, and the instructor also wants separate cash collections and cash disbursements support schedules.");
    assert.equal(analysis.topicFamily?.id, "cash-budget-suite");
    assert.equal(analysis.best?.id, "cash-budget");
    assert.equal(analysis.secondaryRoutes.some((route) => route.id === "cash-collections-schedule"), true);
    assert.equal(analysis.secondaryRoutes.some((route) => route.id === "cash-disbursements-schedule"), true);
...
runTest("smart solver protects intercompany PPE prompts from generic rate contamination", () => {
    const analysis = analyzeSmartInput({ ...INITIAL_FIELDS }, "Parent sold equipment to subsidiary for 260000 when the carrying amount was 200000. The remaining useful life is 5 years. Compute the excess depreciation and the unamortized intercompany profit for year 2.");
...
runTest("smart solver keeps study-first prompts out of false ready-to-open states", () => {
    const analysis = analyzeSmartInput({ ...INITIAL_FIELDS }, "Explain the difference between markup on cost and gross profit on sales, then recommend the lesson I should study before using any calculator.");
    assert.equal(analysis.hasStrongMatch, false);
    assert.equal(analysis.isReadyToRoute, false);
    assert.equal(analysis.warnings.some((warning) => /study or explanation request/i.test(warning)), true);
});
runTest("ocr parser uses topic-first extraction for cash-budget style text", () => {
    ...
    assert.equal(parsed.structuredFields?.some((field) => field.key === "beginningCashBalance"), true);
    assert.equal(parsed.structuredFields?.some((field) => field.key === "beginningCashBalance" && field.normalizedValue === "2026"), false);
});
    ...
    runTest(`smart solver evaluation pack: ${testCase.id}`, () => {
        if (testCase.level === "OCR") {
            ...
            Object.entries(testCase.expectedExtractedFields ?? {}).forEach(([key, value]) => {
                assert.equal(parsed.structuredFields?.some((field) => field.key === key && field.normalizedValue === value), true);
            });
            return;
        }
        ...
        (testCase.expectedSecondaryRoutes ?? []).forEach((routeId) => {
            assert.equal(analysis.secondaryRoutes.some((route) => route.id === routeId), true);
        });
        Object.entries(testCase.expectedExtractedFields ?? {}).forEach(([key, value]) => {
            assert.equal(analysis.extracted[key], value);
        });
    });
});
